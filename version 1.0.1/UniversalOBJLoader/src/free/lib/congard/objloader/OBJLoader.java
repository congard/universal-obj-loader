...
import free.lib.congard.objloader.Model.VerticesDescriptor;
...
public class OBJLoader {
	...
	// [RU] Используется при создании списков с текстурными координатами
	// Например, в JOGL начало координат в левом нижнем углу, а в OpenGL ES в левом верхнем
	// [EN] Used when creating lists with texture coordinates
	// For example, in JOGL the origin in the lower left corner, and in the OpenGL ES in the upper left
	int texA = 0;
	// [RU] Количество вершин для текстуры
	...
	public void load(BufferedReader br) throws IOException {
		...
		while ((line = br.readLine()) != null) {
			...
			case LoaderConstants.OBJ.VERTEX:
				...
				// points
				// max min x
				if (vertex[0] > model.maxX) model.maxX = vertex[0];
				else if (vertex[0] < model.minX) model.minX = vertex[0];
				// max min y
				if (vertex[1] > model.maxY) model.maxY = vertex[1];
				else if (vertex[1] < model.minY) model.minY = vertex[1];
				// max min z
				if (vertex[2] > model.maxZ) model.maxZ = vertex[2];
				else if (vertex[2] < model.minZ) model.minZ = vertex[2];
			...
			case LoaderConstants.OBJ.FACE:
				int[] fv = new int[splitedLine.length-1]; // -1 [RU] Так как 0-й элемент это команда (f), положение вершин
				int[] ft = new int[splitedLine.length-1]; // [RU] положение текстур
				int[] fn = new int[splitedLine.length-1]; // [RU] положение нормалей
				for (int i = 1; i<splitedLine.length; i++) {
					String[] pos = splitedLine[i].split("/");
					if (pos.length == 1) fv[i-1] = Integer.parseInt(pos[0]); // vertex
					else if (pos.length == 2) {
						fv[i-1] = Integer.parseInt(pos[0]); // vertex
						ft[i-1] = Integer.parseInt(pos[1]); // texture coordinate
					} else if (pos.length == 3) {
						fv[i-1] = Integer.parseInt(pos[0]); // vertex
						if (!pos[1].equals("")) ft[i-1] = Integer.parseInt(pos[1]); // texture coordinate
	...
	// [RU] Конвертирует листы в float[] для дальнейшей отрисовки
	...
	public void convertToArrays(boolean isMakeNormalsArray, boolean isMakeTexCoordsArray) {
		...
		for (int i=0;i<faces.size();i++) {
			...
			if (vertd.size() == 0) vertd.add(new Model.VerticesDescriptor());
			...
			for (int w=0; w<tempfaces.length; w++) {
				if (isMakeNormalsArray) if (tempfacesnorms[w] != 0) {
				...
				if (isMakeTexCoordsArray) if (tempfacestexs[w] != 0) {
					textures.add((vertexsetstexs.get(tempfacestexs[w] - 1))[0]);
					textures.add((vertexsetstexs.get(tempfacestexs[w] - 1))[1]);
					if (TEX_MODE == LoaderConstants.TEX_VERTEX_3D) textures.add((vertexsetstexs.get(tempfacestexs[w] - 1))[2]);
				} else {
					textures.add(0f);
					textures.add(0f);
					if (TEX_MODE == LoaderConstants.TEX_VERTEX_3D) textures.add(0f);
				}
		...
		if (isMakeTexCoordsArray) model.texCoords = CgrTools.convertArrayListToArray(textures);
		if (isMakeNormalsArray) model.normals = CgrTools.convertArrayListToArray(normals);
		model.vd = new Model.VerticesDescriptor[vertd.size()];
		for (int i = 0; i<vertd.size(); i++) model.vd[i] = vertd.get(i);
